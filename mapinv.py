...
x, y = m(lats, lons)  # Swap latitudes and longitudes for flipped equator
m.scatter(x, y, color='red', s=50, label="Ancient Sites")

# Plot the Earth's actual equator
equator_lat = np.linspace(-90, 90, 360)  # Flipped equator
equator_lon = [0] * 360
eq_x, eq_y = m(equator_lat, equator_lon)
...
green_eq_x, green_eq_y = m(green_lat_range, green_lon_range)  # Swap latitudes and longitudes for flipped equator
m.plot(green_eq_x, green_eq_y, color='green', linewidth=2, label="Easter Island to Giza Equator")

# Title and legend
plt.title("Top 50 Ancient Sites and Flipped Equators")
...
# Add Earth's flipped equator to KML
equator_line = kml.newlinestring(name="Earth's Flipped Equator")
...
green_line = kml.newlinestring(name="Easter Island to Giza Flipped Equator")
...
# Save KML file
kml.save("flipped_ancient_sites_and_equators.kml")
