...
cart_coords = np.array([latlon_to_cartesian(lat, lon) for lat, lon, name in site_coordinates])

# Define the objective function to fit a plane
def plane_objective_function(params, coords):
    a, b, c, d = params
    x, y, z = coords.T
    return np.sum((a * x + b * y + c * z + d) ** 2)

# Initial guess for the plane coefficients
initial_guess = [1, 1, 1, 1]

# Perform optimization to find the best-fit plane
result = minimize(plane_objective_function, initial_guess, args=(cart_coords,))
a, b, c, d = result.x

# Calculate the intersection of the plane with the sphere (best-fit equator)
lon_range = np.linspace(-180, 180, 360)
lat_range = []
...
# Plot the best-fit equator
eq_x, eq_y = m(lon_range, lat_range)
m.plot(eq_x, eq_y, color='blue', linewidth=2, label="Best-Fit Equator")
...
# Add best-fit equator to KML
line = kml.newlinestring(name="Best-Fit Equator")
line.coords = list(zip(lon_range, lat_range))
